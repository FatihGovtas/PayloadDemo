...
import ApiConfig, { PageUrl, PageEnum } from '../config/ApiConfig';
import { observer, inject } from 'mobx-react';


const Categories = inject("MainStore")(observer(({ MainStore, navigation }) => {

  const itemsPerPage = 4; // Her sayfada gösterilecek öğe sayısı

  useEffect(() => {
    ApiConfig.getUser(navigation);
    ApiConfig.getData(PageUrl.Categories);
  }, []);
  ...
  const filteredData = MainStore.data.filter((item) => {
    console.log(item)
    return item.name.toLowerCase().includes(MainStore.searchQuery.toLowerCase())
  }
        ...
        <ScrollView horizontal style={styles.container}>
          <DataTable style={{ width: 700 }}>
            <DataTable.Header style={styles.table_title}>
              ...
              <DataTable.Title onPress={() => sortData('archived')} sortDirection={MainStore.sortedField === 'archived' ? (MainStore.sortAscending ? 'ascending' : 'descending') : 'none'}>
                Archived
              </DataTable.Title>
            </DataTable.Header>
            ...
            {filteredData.slice(MainStore.page * itemsPerPage, (MainStore.page + 1) * itemsPerPage).map((item) => (
              <DataTable.Row key={item.id} style={{ justifyContent: 'center' }}>
                ...
                <DataTable.Cell>{item.archived.toString()}</DataTable.Cell>
              </DataTable.Row>
            ))}
          </DataTable>
        </ScrollView>
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <TouchableOpacity style={styles.button_style} onPress={() => {
                ApiConfig.deleteAlert(MainStore.selectedItems, PageUrl.Categories, PageEnum.Categories);
              }}>
