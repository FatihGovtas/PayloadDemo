...
const FormSubmissions = inject("MainStore")(observer(({ MainStore, navigation }) => {

  const itemsPerPage = 4; // Her sayfada gösterilecek öğe sayısı

  useEffect(() => {
    ApiConfig.getUser(navigation);
    ApiConfig.getData(PageUrl.FormSubmissions);
  }, []);
  ...
  const filteredData = MainStore.data.filter((item) => item.id.toLowerCase().includes(MainStore.searchQuery.toLowerCase())
  ).sort((a, b) => {
    ...
    <View style={{ paddingHorizontal: 15, backgroundColor: theme.colors.background, flex: 1 }}>
      <ScrollView refreshControl={
        ...
        <View style={{ flexDirection: 'row', marginVertical: 15, alignItems: 'center', }}>
          <Text style={styles.title_style}>Form Submissions</Text>
        ...
        <View style={styles.searchBar}>
          ...
          <TextInput
            style={styles.input}
            placeholder="Şuna göre sırala: ID"
        ...
        <ScrollView horizontal style={styles.container}>
          <DataTable style={{ width: 700 }}>
            <DataTable.Header style={styles.table_title}>
              ...
              <DataTable.Title onPress={() => sortData('submissionData')} sortDirection={MainStore.sortedField === 'submissionData' ? (MainStore.sortAscending ? 'ascending' : 'descending') : 'none'}>
                Submission Data
              </DataTable.Title>
            </DataTable.Header>
            ...
            {filteredData.slice(MainStore.page * itemsPerPage, (MainStore.page + 1) * itemsPerPage).map((item) => (
              <DataTable.Row key={item.id} style={{ justifyContent: 'center' }}>
                ...
                <DataTable.Cell>{item.form.title}</DataTable.Cell>
                <DataTable.Cell>{item.submissionData.length.toString()}</DataTable.Cell>
              </DataTable.Row>
            ))}
          </DataTable>
        </ScrollView>
          ...
          <View>
            ...
            <View style={{ flexDirection: 'row' }}>
              ...
              <TouchableOpacity style={styles.button_style} onPress={() => {
                ApiConfig.deleteAlert(MainStore.selectedItems, PageUrl.FormSubmissions, PageEnum.FormSubmissions);
              }}>
